...
with open('src/scripts/modules/hero-particles.js', 'r') as f:
    content = f.read()
...
content = re.sub(
    r"drawColor = `rgba\(56, 189, 248, \$\{irisBrightness\}\)`;",
    r"drawColor = isLight ? `rgba(14, 165, 233, ${irisBrightness})` : `rgba(56, 189, 248, ${irisBrightness})`;",
    content
)
content = content.replace(
...
robot_state_old = '''  // --- ROBOT AI STATE ---
...
aicore_state_new = '''  // --- AI CORE STATE ---
  const CORE_SIZE = 60;
  const RING1_SIZE = 60;
  const RING2_SIZE = 60;
...
update_robot_old = '''    particles.forEach((p, idx) => {
      let tx, ty;
...
update_robot_new = '''    particles.forEach((p, idx) => {
      let tx, ty;
...
content = re.sub(
    r"// Faint glow circle around each eye centre.*?\n\s+\}\);", 
    "", 
    content, 
    flags=re.DOTALL
)
...
with open('src/scripts/modules/hero-particles.js', 'w') as f:
    f.write(content)
