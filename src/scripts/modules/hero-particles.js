...
export function initHeroParticles(options = {}) {
  ...
  // Performance-friendly early exit
  if (perfLite || isMobile || prefersReducedMotion) {
  ...
  function updateRobotMode() {
    ...
    if (mouse.x !== null && mouse.y !== null) {
      ...
      const maxLook = 20;
      lookX = (dx / dist) * Math.min(maxLook, dist * 0.06);
      lookY = (dy / dist) * Math.min(maxLook, dist * 0.06);
    }
    ...
    particles.forEach((p, idx) => {
      ...
      if (idx < 100) {
        // Head outline (squircle shape)
        const angle = (idx / 100) * Math.PI * 2;
        const w = 240;
        const h = 185;
        const n = 3; // squircle degree
        const cos = Math.cos(angle);
        const sin = Math.sin(angle);
        const sx = Math.sign(cos) * Math.pow(Math.abs(cos), 2 / n);
        const sy = Math.sign(sin) * Math.pow(Math.abs(sin), 2 / n);

        tx = width / 2 + sx * w + lookX * 0.25;
        ty = height / 2 + sy * h + lookY * 0.25;
      } else if (idx < 135) {
        // Left Eye
        const angle = ((idx - 100) / 35) * Math.PI * 2;
        const r = 26;
        const eyeCX = width / 2 - 80 + lookX;
        const eyeCY = height / 2 - 25 + lookY;
        ...
        // Right Eye
        const angle = ((idx - 135) / 35) * Math.PI * 2;
        const r = 26;
        const eyeCX = width / 2 + 80 + lookX;
        const eyeCY = height / 2 - 25 + lookY;
        ...
        // Smiling Mouth
        const progress = (idx - 170) / 29;
        const mouthW = 120;
        const xOffset = -mouthW / 2 + progress * mouthW;
        const yOffset = 45 + (xOffset * xOffset) * 0.0022;

        tx = width / 2 + xOffset + lookX * 0.5;
        ty = height / 2 + yOffset + lookY * 0.5;
      }
  ...
  document.addEventListener('mouseover', handleDocumentOver, { passive: true });
  document.addEventListener('mouseout', handleDocumentOut, { passive: true });
  document.addEventListener('mousemove', handleMouseMove, { passive: true });
  document.addEventListener('click', handleClickBurst, { passive: true });
