...
// Hero spotlight: smooth, inertial spotlight with subtle color tint
function bindHeroSpotlight() {
  ...
  // Respect class-defined mix-blend mode from HTML/CSS
  ...
  const setGradient = (x, y, extra = 0) => {
    ...
    // Hue shifts slightly with horizontal position for a subtle tint
    const rect = hero.getBoundingClientRect();
    const nx = Math.min(1, Math.max(0, x / Math.max(1, rect.width)));
    const hue = Math.round(200 + nx * 60); // 200-260 (blue to indigo)
  ...
  const animate = () => {
    ...
    // Extra radius based on velocity magnitude
    ...
    const extra = speed * 0.25; // subtle expansion on faster movement
